...
var createDOMElement = function createDOMElement(vNode, $parent, $old) {
  var type = _typeof(vNode);

  if (type === 'string' || type === 'number') {
    var textNode = document.createTextNode(vNode);
    $old ? $parent.replaceChild(textNode, $old) : $parent.appendChild(textNode);
    return textNode;
  } else {
...
var renderNode = function renderNode(_ref) {
  ...
  for (var _i = 0, _Object$entries = Object.entries(attrs); _i < _Object$entries.length; _i++) {
    ...
    if (typeof v === 'function') {
      var _update = updateAttr.bind(null, $el, k, v);
  ...
  try {
    var _loop = function _loop() {
      ...
      if (type === 'function') {
        var c, $child; // let $child = createDOMElement(c, $el);
  ...
  return function ($parent, $old) {
    // update();
    $old ? $parent.replaceChild($el, $old) : $parent.appendChild($el);
    return $el; // return [ $el, update ];
  };
};

var componentExample = function componentExample() {
  var _ref2 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
      createState = _ref2.createState;
  ...
  return h('div', {
    ...
    return "".concat(state.greeting, " world");
  }, '!'), h('h3', {}, 'Hi, Dane!!!!!') // ({state}) => `${state.greeting} world!`
  );
